...
脚本在 skill 目录的 `scripts/` 下。Agent 根据自己安装位置调用，不要切换目录。输出默认到当前目录的 `./output/` 下，可通过 `--output-dir` 覆盖。**禁止将图片输出到 skill 目录内**——如果当前目录恰好在 skill 安装路径下，必须通过 `--output-dir` 指定到其他位置。
